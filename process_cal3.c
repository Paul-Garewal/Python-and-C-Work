...
event_t *new_event(event_t *event)
{
    ...
    assert( event != NULL); // assert if null event
    
    event_t *temp = (event_t *)emalloc(sizeof(event_t)); // free up dynamic memory in new event

    // sent new node with dynamic memory to the appropriate values from the event argument passed in
    ...
    return temp; // return new event  node with dynamic memory allocated
}
...
void print_formatted_date(char *date)
{
    ...
    int dashCount = 0; // initialize dashCount

    dashCount += strlen(date); // set dashCount to length of the date string

    printf("%s\n", date); // print date

    // print dashes based on number of dashCounts
    for(int i = 0; i<dashCount; i++){

        if(i+1 == dashCount){
            printf("%s\n", "-");// if last dashcount, print new line and break out of the loop
            break;
        }
        printf("%s", "-"); // print dash each iteration of the loop
    }
...
{
    // print date in required format using printf()
