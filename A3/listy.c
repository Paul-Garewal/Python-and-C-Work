...
{
    // confirm argument being passed is not NULL
    assert(val != NULL);
   
   // dynamically allocate memory for new node 
    node_t *temp = (node_t *)emalloc(sizeof(node_t));
    
    // assign values to attributes associated with the new node
   temp->val = val;
   temp->next = NULL;
...
node_t *add_inorder(node_t *list, node_t *new)
{
    ...
    // if list is empty, simply return new 
    if(list == NULL){
        return new;
    }

    // loop through linked list, reordering node where appropriate by date 
    ...
    new->next = curr; // set new pointer to curr

    // set prev and return list, return new is prev is null 
    if(prev == NULL) {
        return (new);
    } else{
        prev->next = new;
